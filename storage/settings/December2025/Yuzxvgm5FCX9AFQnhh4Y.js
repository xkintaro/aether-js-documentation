/**
 * AETHER JS - Headless & Utility-First Interaction Controller
 * Version: 1.0.0
 * License: MIT
 * Repo: https://github.com/xkintaro/aether-js
 */
...
const AetherUI = {
    ...
    open(id, triggerEl = null) {
        ...
        el.querySelectorAll(this._sel("active-class")).forEach((c) => {
            if (this._has(c, "click-outside") || this._has(c, "scroll-lock"))
                return;
    ...
    close(id) {
        ...
        if (
            state.returnFocusTo &&
            document.body.contains(state.returnFocusTo)
        ) {
            ...
            if (document.documentElement.scrollHeight > window.innerHeight) {
                if (this._scrollbarWidth === undefined) {
                    ...
                    this._scrollbarWidth =
                        outer.offsetWidth - outer.clientWidth || 0;
                    outer.remove();
                }
        ...
        this.observer = new MutationObserver((mutations) => {
            ...
            const shouldRefresh = mutations.some(
                (m) =>
                    m.type === "childList" ||
                    (m.type === "attributes" &&
                        m.attributeName.startsWith(this._pfx))
            );
            if (shouldRefresh && this._refreshDebounced)
    ...
    initTabs() {
        document
            .querySelectorAll(
                `${this._sel("ui", "tab-group")}:not([data-init])`
            )
            .forEach((g) => {
                ...
                const def =
                    triggers.find((t) => this._has(t, "default-open")) ||
                    triggers[0];
    ...
    activateTab(t, skip = false) {
        ...
        if (active) {
            ...
            if (this.config.syncUrl && this._has(g, "sync-url"))
                this.updateHash(id);
        }
    },
    ...
    initAccordions() {
        document
            ...
            .forEach((g) => {
                ...
                const defs = triggers.filter((t) =>
                    this._has(t, "default-open")
                );
                const panels = this.getScopedChildren(
                    g,
        ...
        if (active && !this._has(g, "allow-multiple")) {
            this.getScopedChildren(
                ...
                .forEach((ot) => {
                    ...
                    const op = document.getElementById(
                        this._attr(ot, "target")
                    );
                    if (op) {
    ...
    handleGlobalClick(e) {
        if (e.target.closest(this._sel("theme")))
            return this.setTheme(
                e.target
                    .closest(this._sel("theme"))
                    .getAttribute(`${this._pfx}-theme`)
            );
        ...
        if (
            ...
            !e.target.closest(
                `${this._sel("trigger")}${this._sel("target", last)}`
            )
        ) {
            this.close(last);
        }
        ...
        const autoInit = () => {
            if (
                ...
                document.body.hasAttribute(
                    `data-${AetherUI.config.prefix}-auto`
                )
            )
                AetherUI.init();
        };
